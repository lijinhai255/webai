; (function (window, voice) {
    ...
    return class IatRecorder {
        ...
        // 设置识别结果内容
        setResultText({ status, resultText, resultTextTemp } = {}) {
            this.onTextChange && this.onTextChange(status, resultTextTemp || resultText || '');
        ...
        start() {
            this.recorderStart();
            this.setResultText({ status: -1, resultText: '', resultTextTemp: '' });
        };
        // 停止
        stop() {
            this.recorderStop();
